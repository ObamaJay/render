...
# ---------------- Text helpers ----------------
CTRL_RE = re.compile(r"[\x00-\x08\x0B\x0C\x0E-\x1F]")  # strip control chars
...
def normalize_spaces(s: str) -> str:
    ...
    s = CTRL_RE.sub("", s)
    # soften absurd runs of dashes/underscores to keep layout sane
...
def soften_long_tokens(s: str, max_len: int = 40) -> str:
    ...
    # Then any overlong token
    return re.sub(r"\S{" + str(max_len) + r",}", lambda m: chunker(m.group(0)), s)

def sanitize_text(text: str) -> str:
    """Normalize Unicode punctuation to ASCII so FPDF core fonts won't choke."""
    text = (text or "")
    text = (text
...
class PDF(FPDF):
    ...
    'Page X/{nb} · Generated by ImmigrAI.org - Not affiliated with USCIS or the U.S. government.'
    """
    def footer(self):
        self.set_y(-15)
...
def safe_multicell(pdf: PDF, line: str, line_height: float, avail_w: float):
    try:
        ...
        # Last-resort chunking if FPDF still complains
        chunk = 60
        i = 0
        while i < len(line):
...
def stripe_webhook():
    ...
    try:
        ...
        # 6) Write back to lead (by id if possible)
        try:
            update_payload = {
        ...
        # 7) Optional: log payment
        amount = int(session.get("amount_total") or 1900)  # cents
        ...
        traceback.print_exc()
        # Return 200 so Stripe doesn’t retry forever; logs will show the exact issue
